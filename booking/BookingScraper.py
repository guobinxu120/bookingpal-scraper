...
def run_crawler(days, source_file_path):
	if "*" in source_file_path:
		...
		source_file_list = [f for f in source_file_list if "booking" in f]
	else:
	...
	for csv_file in source_file_list:
		...
		path = "{}/booking/".format(datetime.date.today().strftime("%Y%m%d"))
		...
		for index, record in enumerate(csv_base):		
			if index == 0:
				continue
			param = "\"" + '---'.join(record) + "\""		
			spider_name_params.append(['booking', days, param, root+path, index])
		...
		#pool.map(_crawl, spider_name_params[:12])
...
if __name__ == '__main__':
	if len(sys.argv) != 3:
		print "Usage: python BookingScraper.py <Days> <CSV file path>"
	else:
